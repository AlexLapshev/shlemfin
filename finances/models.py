...
class Debt(models.Model):
    ...
    debt_active = models.BooleanField('Погашен', default=False)
...
class SendTo(models.Model):
    fcs = models.CharField('ФИО', max_length=120)
    where = models.CharField('Адрес', max_length=300)
    send_description = models.CharField('Дополнительная Информация', max_length=120, blank=True, null=True)
    send_done = models.BooleanField('Выполнение', default=False)

    def __str__(self):
        return '{} {}'.format(self.fcs, self.where)

    class Meta:
        verbose_name = 'Куда Отправлять'
        verbose_name_plural = 'Куда Отправлять'
